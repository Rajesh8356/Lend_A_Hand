...
def userreg():
    if request.method == 'POST':                                                                    
        ...
        try:
            ...
            # CORRECTED INSERT STATEMENT - removed kannada_text and english_text
...
def api_rent_requests():
    ...
    try:
        # First get vendor's business name
        conn_vendors = sqlite3.connect('vendors.db')
        c_vendors = conn_vendors.cursor()
        c_vendors.execute("SELECT business_name FROM vendors WHERE id = ?", (vendor_id,))
        vendor = c_vendors.fetchone()
        conn_vendors.close()
        ...
        # Get rent requests for this vendor (using vendor name instead of ID)
        conn_equipment = sqlite3.connect('equipment.db')
        conn_equipment.row_factory = sqlite3.Row
        c_equipment = conn_equipment.cursor()
        
        c_equipment.execute('''SELECT 
                             id, farmer_name, farmer_phone, equipment_name, 
                             start_date, end_date, duration, purpose, notes,
                             total_amount, status, submitted_date, processed_date
                             FROM rent_requests 
                             WHERE vendor_name = ? 
                             ORDER BY submitted_date DESC''', (vendor_name,))
        
        requests = c_equipment.fetchall()
...
def api_update_rent_request(request_id, action):
    ...
    try:
        ...
        conn_vendors = sqlite3.connect('vendors.db')
        c_vendors = conn_vendors.cursor()
        c_vendors.execute("SELECT business_name FROM vendors WHERE id = ?", (session['vendor_id'],))
        vendor = c_vendors.fetchone()
        conn_vendors.close()
        ...
        # Verify the request belongs to this vendor (using vendor name)
        c_equipment.execute('''SELECT * FROM rent_requests WHERE id = ? AND vendor_name = ?''', 
                          (request_id, vendor_name))
        
        request_data = c_equipment.fetchone()
        if not request_data:
            conn_equipment.close()
        ...
        c_equipment.execute('''UPDATE rent_requests 
        ...
        farmer_phone = request_data[2]  # farmer_phone is at index 2
        status_text = "approved" if action == "approve" else "rejected"
        sms_message = f"Your rent request for {request_data[3]} has been {status_text} by the vendor."
...
def api_submit_rent_request():
    ...
    try:
        data = request.get_json()
        print("🔍 Received rent request data:", data)
        ...
        # Get farmer details (only name and phone - no ID)
        conn_agri = sqlite3.connect('agriculture.db')
        c_agri = conn_agri.cursor()
        ...
        # Get equipment and vendor details (only names - no IDs)
        conn_equipment = sqlite3.connect('equipment.db')
        conn_equipment.row_factory = sqlite3.Row
        c_equipment = conn_equipment.cursor()
        
        c_equipment.execute('''SELECT e.name as equipment_name, e.price, v.business_name as vendor_name
                              FROM equipment e 
        ...
        base_amount = equipment_price * duration
        service_fee = base_amount * 0.1
        total_amount = base_amount + service_fee
        
        print(f"💰 Rental Details: {duration} days, Total: ₹{total_amount}")
        
        # Insert rent request with only relevant information (no technical IDs)
        c_equipment.execute('''INSERT INTO rent_requests 
                             (farmer_name, farmer_phone, equipment_name, vendor_name,
                              start_date, end_date, duration, purpose, notes,
                              base_amount, service_fee, total_amount, status)
                             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''',
                             (farmer_name, farmer_phone, equipment['equipment_name'], 
                              equipment['vendor_name'], start_date, end_date, duration, 
                              data['purpose'], data.get('notes', ''), base_amount, 
                              service_fee, total_amount, 'pending'))
        
        conn_equipment.commit()
        request_id = c_equipment.lastrowid
        conn_equipment.close()
        
        print(f"✅ Rent request stored successfully! ID: {request_id}")
        
        # Send SMS to vendor (using vendor name instead of ID)
        try:
            vendor_conn = sqlite3.connect('vendors.db')
            vendor_c = vendor_conn.cursor()
            vendor_c.execute("SELECT phone FROM vendors WHERE business_name = ?", (equipment['vendor_name'],))
            vendor_phone_result = vendor_c.fetchone()
            vendor_conn.close()
            
            if vendor_phone_result:
                vendor_phone = vendor_phone_result[0]
                sms_message = f"New rent request for {equipment['equipment_name']} from {farmer_name}. Check your dashboard."
                send_sms(vendor_phone, sms_message)
                print(f"📱 SMS sent to {equipment['vendor_name']}")
        except Exception as sms_error:
            print(f"⚠️ SMS notification failed: {sms_error}")
        ...
        print(f"❌ Error in submit-rent-request: {str(e)}")
        return jsonify({'error': str(e)}), 500
# Debug endpoint to check rent requests in database
@app.route('/debug/rent-requests')
def debug_rent_requests():
    conn = sqlite3.connect('equipment.db')
    c = conn.cursor()
    ...
    return jsonify({
        ...
        'requests': [dict(row) for row in all_requests] if all_requests else []
    })
